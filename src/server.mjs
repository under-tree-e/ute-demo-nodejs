...
app.set('views', [path.join(path.resolve(), 'views'), path.join(path.resolve(), 'todo')])
app.set('view engine', 'ejs')
app.use(express.static(path.join(path.resolve(), 'public')))

const sessionSecret = process.env.SESSION_SECRET || (!isProduction ? packageJson.name : '')
if (!sessionSecret) {
...
if (process.env.NODE_ENV !== 'test') {
  app.use(
    logger('dev', {
      skip: (req) => req.path.indexOf('/signin') === 0 || req.path === '/healthz' || req.path === '/readyz',
    }),
  )
}
